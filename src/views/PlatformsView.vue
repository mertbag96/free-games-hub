...
import { GAME_PLATFORMS } from '@/data/gameFilters.js'
import { ComputerDesktopIcon, MagnifyingGlassIcon } from '@heroicons/vue/24/outline'

const platformSearch = ref('')

const visiblePlatforms = computed(() => {
    const q = platformSearch.value.trim().toLowerCase()
    if (!q) return GAME_PLATFORMS

    return GAME_PLATFORMS.filter(
        (p) =>
            p.label.toLowerCase().includes(q) ||
            p.value.toLowerCase().includes(q) ||
            p.value.replace(/-/g, ' ').toLowerCase().includes(q),
    )
})
</script>
...
<template>
    <PageContainer class="py-8 md:py-10">
        <SectionHeading title="Browse by platform" :suffix="visiblePlatforms.length" />

        <p class="mb-6 max-w-2xl text-sm leading-relaxed text-slate-600 md:text-[0.9375rem]">
            Pick where you prefer to play to open the
            <RouterLink
            ...
            with that platform pre-selected. Options match what you'll see on the Browse games filters.
        </p>
        ...
        <section
            ...
            aria-label="Quick platform finder"
        >
            <div
            ...
            <div class="relative w-full space-y-4">
                <FormField
                    ...
                    variant="filter"
                    label="Spot a platform instantly"
                    type="search"
                ...
                <p
                    v-if="platformSearch.trim()"
                    class="text-xs font-medium leading-relaxed text-slate-500"
                    role="status"
                    aria-live="polite"
                >
                    Showing {{ visiblePlatforms.length }}
                    {{ visiblePlatforms.length === 1 ? 'platform' : 'platforms' }}. Clear the box to expose all
                    {{ GAME_PLATFORMS.length }} presets.
                </p>
                <p v-else class="text-xs leading-relaxed text-slate-500">
                    {{ GAME_PLATFORMS.length }} presets below mirror the selectors on Browse games.
                </p>
            </div>
        </section>

        <div v-if="visiblePlatforms.length > 0" id="platforms-list">
            <ul class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                <li v-for="plat in visiblePlatforms" :key="plat.value">
                    <RouterLink
                        :to="{ name: 'games', query: { platform: plat.value } }"
                        ...
                        :aria-label="`Browse ${plat.label} games`"
                    >
                        <span
                        ...
                        >
                            <ComputerDesktopIcon class="size-[1.35rem]" aria-hidden="true" />
                        </span>
                        <span class="min-w-0 truncate text-sm font-semibold">{{ plat.label }}</span>
                        <span
        ...
        <div
            ...
            role="status"
        >
            No platforms match &quot;{{ platformSearch.trim() }}&quot;. Try a shorter or different spelling.
        </div>
    </PageContainer>
</template>
